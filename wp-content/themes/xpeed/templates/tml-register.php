...
<main class="auth-wrapper">
    <div class="auth-wrapper__container">
        ...
        <form class="auth-wrapper__form" id="register-form">
            <input
            ...
            <input
                    class="auth-wrapper__input auth-wrapper__input--name auth-wrapper__input--readonly"
                    type="text"
                    placeholder="우편번호"
                    readonly
            />
        ...
        <div class="auth-wrapper__links auth-wrapper__links--register">
            <div class="auth-wrapper__divider">
                ...
                <div class="auth-wrapper__social-buttons">
                    ...
                    <button
                            class="auth-wrapper__social-button auth-wrapper__social-button--kakao"
                            type="button"
                    >
                        <svg
                                width="16"
                    ...
                    <button
                            class="auth-wrapper__social-button auth-wrapper__social-button--naver"
                            type="button"
                    >
                        <svg
                                width="16"
...
<script>
    document.getElementById('register-form').addEventListener('submit', function (e) {
        ...
        const messageDiv = document.getElementById('register-message'); // Lấy phần thông báo

        // Gửi yêu cầu đến API đăng ký
        fetch('<?php echo esc_url(rest_url('custom-api/v1/register')); ?>', {
            method: 'POST',
            body: JSON.stringify({
            ...
            .then(data => {
                ...
                if (data.code === 200) {
                    messageDiv.style.color = 'green'; // Đặt màu xanh cho thông báo thành công
                    messageDiv.textContent = data.message; // Hiển thị thông báo thành công
                    // Chuyển hướng hoặc thực hiện hành động khác
                } else {
                    messageDiv.style.color = 'red'; // Đặt màu đỏ cho thông báo lỗi
                    messageDiv.textContent = data.message; // Hiển thị thông báo lỗi
                }
            })
            .catch(error => {
